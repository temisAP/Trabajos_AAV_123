...
# Coeficiente balístico y eficiencia según el ángulo de vuelo (FLUENT)
...
Viking1.epsilon = 5     # Error admisible
...
if trayectoria == 'yes':
    x = Viking1.r*sin(Viking1.theta)/1e3
    y = Viking1.r*cos(Viking1.theta)/1e3

    fig = plt.figure()
    plt.plot(x,y-Viking1.RT/1e3)
    plt.grid()
    plt.xlabel('x [km]')
    plt.ylabel('Altitud [km]')
    ...
    fig, ax = plt.subplots()
    Tierra = plt.Circle((0, 0), Viking1.RT/1e3, color='b')
    ax.add_patch(Tierra)
    plt.plot(x,y)
    plt.grid()
...
Viking1.reentrada_analitica([Ue,gamma_e,z0,theta0])
